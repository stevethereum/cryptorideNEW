...
contract cryptoRide is usingOraclize {
  ...
  */
  ...
  modifier whenOngoing {
      require(keccak256(abi.encodePacked(challengeStatus)) == keccak256(abi.encodePacked('ongoing')));
      _;
  }
  // For functions that can only be executed when the challenge is accomplished
  modifier whenAccomplished {
      require (keccak256(abi.encodePacked(challengeStatus)) ==  keccak256(abi.encodePacked('accomplished')));
      _;
  }
  // For functions that can only be executed when the challenge is failed
  modifier whenFailed {
      require (keccak256(abi.encodePacked(challengeStatus)) ==  keccak256(abi.encodePacked('failed')));
      _;
  }
  ...
  */
  ...
  constructor(address _PelotoniaAddress) public {
  ...
  function __callback(bytes32 _queryId, string _result) public onlyOracle {
    ...
    if  (keccak256(abi.encodePacked(latestStatus)) ==  keccak256(abi.encodePacked('ongoing'))) {
      challengeStatus = 'ongoing';
      emit LogChallengeOngoing();
    } else if  (keccak256(abi.encodePacked(latestStatus)) ==  keccak256(abi.encodePacked('accomplished'))) {
      challengeStatus = 'accomplished';
      emit LogChallengeAccomplished();
      emit LogDonationAvailableForPelotoniaWithdrawal(totalDonation);
    } else if  (keccak256(abi.encodePacked(latestStatus)) ==  keccak256(abi.encodePacked('failed'))) {
